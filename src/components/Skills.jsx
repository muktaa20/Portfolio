...
const Skills = () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 py-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h1
            className={`text-5xl md:text-6xl lg:text-7xl font-bold text-blue-400 mb-8 transition-all duration-1000 ${
              isVisible
          ...
          {skills.map((skill, index) => (
            ...
            >
              <div className="text-center mb-6">
                <div
                  className={`flex justify-center items-center mb-4 transform transition-all duration-300 ${
  ...
  onError={(e) => {
    console.error(`Failed to load image for ${skill.name}:`, e);
    // Use a local fallback image or default icon
    e.target.src = "/default-skill-icon.png"; // ✅ Make sure this exists in your public/ folder
  }}
/>
              ...
              <div className="absolute inset-0 overflow-hidden rounded-3xl pointer-events-none">
                <div
                  className={`absolute top-4 right-4 w-2 h-2 ${skill.color.replace(
                    "text-",
                    "bg-"
                  )} rounded-full opacity-20 transition-all duration-500 ${
                    hoveredCard === index ? "scale-150 opacity-40" : ""
                  }`}
                ></div>
                <div
                  className={`absolute bottom-4 left-4 w-1 h-1 ${skill.color.replace(
                    "text-",
                    "bg-"
                  )} rounded-full opacity-30 transition-all duration-700 ${
        ...
        <div className="fixed inset-0 overflow-hidden pointer-events-none">
          ...
          <div
            className="absolute top-1/2 right-1/3 w-5 h-5 bg-yellow-400 rounded-full opacity-15 animate-bounce"
